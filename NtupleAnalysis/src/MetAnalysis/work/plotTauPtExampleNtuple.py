...
def dataMCExample(datasets):
 ...
 #   plots.drawPlot(plot, "Met", xlabel="E_{T}^{miss} (GeV)", ylabel="Number of events",
#                   rebin=1, stackMCHistograms=True, addMCUncertainty=True, addLuminosityText=True,
#                   opts={"ymin": 1e-1, "ymaxfactor": 10}, log=True)


def MetComparison(datasets):
    mt = plots.PlotBase([
        datasets.getDataset("TTJets").getDatasetRootHisto("MetNoJetInHole"),
        datasets.getDataset("TTJets").getDatasetRootHisto("MetJetInHole"),
        ])
...
#    mt.drawPlot(plot, "tauEta", xlabel="Tau p_{T} (GeV/c)", ylabel="Number of events",
#                   rebin=1, stackMCHistograms=True, addMCUncertainty=True, addLuminosityText=True,
#                   opts={"ymin": 1e-1, "ymaxfactor": 10}, log=True)
