...
def main():
    ...
    for counter, hName in enumerate(hNames):
        ...
        if doEfficiency:
            saveName = os.path.join(savePath, hName + "_Efficiency")
            effGreaterThan = getCutEfficiency(datasets.getMCDatasets(), hName, ">")
            effLessThan    = getCutEfficiency(datasets.getMCDatasets(), hName, "<")
            p = plots.ComparisonPlot(histograms.HistoGraph(effGreaterThan, ">", "p", "P"),
                                     histograms.HistoGraph(effLessThan   , "<", "p", "P"))
        else:
...
#================================================================================================
def getCutEfficiency(datasets, histoName, cutDirection=">"):
    ...
    for dataset in datasets:
        ...
        hLine    = "="*70
        msgAlign = '{:<4} {:<23} {:<23} {:<20}'
        title    = msgAlign.format("i", "Pass", "Total", "Efficiency")
        table.append(hLine)
        table.append(title)
        table.append(hLine)
        ...
        for iBin in range(1, nBinsX):
            ...
            eff    = nPass/nTotal
            values = msgAlign.format(iBin, nPass, nTotal, eff)
            table.append(values)
        table.append(hLine)
    ...
    if cutDirection != ">":
        style = styles.effGreaterThanStyle
    else:        
        style = styles.effLessThanStyle        
    return convert2TGraph(teff, style )
...
def convert2TGraph(tefficiency, style):
    x     = []
    y     = []
    xerrl = []
    xerrh = []
