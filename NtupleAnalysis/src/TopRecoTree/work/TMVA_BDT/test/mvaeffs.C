...
public:
  ...
  void SetResultHists() 
  {
    ...
    for (int i=0; i<nbins; i ++){
      double yS = sigE->GetBinContent(i);
      double yB = bgdE->GetBinContent(i);
      double binW = bgdE->GetBinWidth(i);
      double x_value = sigE->GetXaxis()->GetBinCenter(i);
      if (x_value < 0.40 + binW && x_value > 0.40 - binW){
	std::cout<<"Efficiency when cutting at "<<x_value<<std::endl;
	std::cout<<"signal "<<yS<<std::endl;
	std::cout<<"background "<<yB<<std::endl;
      }
    }
...
class StatDialogMVAEffs {  
  ...
  void DrawHistograms(TString fin = "");
...
void StatDialogMVAEffs::UpdateSignificanceHists() 
{
  ...
  while ((info = (MethodInfo*)next())) {
    ...
    //info->sSig->Scale(1/info->maxSignificance); soti
...
void StatDialogMVAEffs::ReadHistograms(TFile* file) 
{
  ...
  while( (key = (TKey*)next()) ) {
    ...
    while((titkey = (TKey*)keyIt())) {
      ...
      GetMethodName(info->methodName,key);
      GetMethodTitle(info->methodTitle,titDir);
...
void StatDialogMVAEffs::DrawHistograms(TString fin = "") 
{
  // counter variables
  Int_t countCanvas = 0;
  ...
  MethodInfo* info(0);
   while ( (info = (MethodInfo*)next()) ) {
     ...
     fin.ReplaceAll("TopRecoTree", "mvaeffs");
     
     TFile* outfile=new TFile(fin,"RECREATE");
     outfile -> cd();
     ...
     std::cout<<Form("For %1.0f signal and %1.0f background", fNSignal, fNBackground)<<std::endl;
     std::cout<<Form("%4.2f the maximum significance when cutting at %5.2f", info->maxSignificance, info->sSig->GetXaxis()->GetBinCenter(maxbin))<<std::endl;
     ...
     if (Save_Images) {
       ...
       imgconv( c, Form("plots/mvaeffs_%s", info->methodTitle.Data()) );
     }
     countCanvas++;
   }
}
              ...
              Bool_t useTMVAStyle = kTRUE, TString formula="S/sqrt(S+B)" )
{
  ...
  gGui->DrawHistograms(fin);
  gGui->RaiseDialog();      
}

void StatDialogMVAEffs::GetMethodName( TString & name, TKey * mkey ) {
    if (mkey==0) return;
    name = mkey->GetName();
    name.ReplaceAll("Method_","");
  }
  
  void StatDialogMVAEffs::GetMethodTitle( TString & name, TKey * ikey ) {
    if (ikey==0) return;
    name = ikey->GetName();
  }

  void StatDialogMVAEffs::GetMethodName( TString & name, TDirectory * mdir ) {
    if (mdir==0) return;
    name = mdir->GetName();
    name.ReplaceAll("Method_","");
  }
  
  void StatDialogMVAEffs::GetMethodTitle( TString & name, TDirectory * idir ) {
    if (idir==0) return;
    name = idir->GetName();
  }

  void StatDialogMVAEffs::DestroyCanvases()
  {
  ...
  void StatDialogMVAEffs::Initialize( Bool_t useTMVAStyle = kTRUE )
  {
    // destroy canvas'
    DestroyCanvases();
  ...
  void StatDialogMVAEffs::imgconv( TCanvas* c, const TString & fname )
  {
    // return;
    if (NULL == c) {
	...
	cout << "--- the corresponding lines n mvaeffs.C" << endl;
	cout << "--- --------------------------------------------------------------------" << endl;
	c->Print(epsName);
	c->Print(pdfName);
	// c->Print(gifName);
