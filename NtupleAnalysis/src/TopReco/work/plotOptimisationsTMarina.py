...
#          eventsNo.SetBinError(iBin, math.sqrt(nPass)/10)
          eventsNo.SetBinError(iBin, sqrt(nPass)/10)
...
def main():
     ...
     drawStyle    = kwargs.get("drawStyle")
     legStyle     = kwargs.get("legStyle")
     legName180   = plots._legendLabels[Signal_Dataset_180.getName()]
     ...
     for counter, hName in enumerate(hNames):
          ...
          rootHisto_180 = Signal_Dataset_180.getDatasetRootHisto(hName)
          ...
          NormalizeRootHisto(Signal_Dataset_180, rootHisto_180, Signal_Dataset_180.isMC(), "One")               
          ...
          histo180 = rootHisto_180.getHistogram()
          ...
          # Customize Histos 
          styles.signal180Style.apply(histo180)
          ...
          binWidth = histo180.GetBinWidth(0)


          # Get Histos for Plotter                                                                 
          signal_histo_180 = histograms.Histo(histo180, legName180, legStyle, "p", "P")  #, "F", "HIST9")
          ...
          for background_histo in [background1_histo,background2_histo, background3_histo, background4_histo]:
               ...
               p = plots.ComparisonManyPlot(background_histo,[signal_histo_300, signal_histo_500, signal_histo_180, signal_histo_800, signal_histo_1000])
     ...
     for Background_Dataset in [Background1_Dataset, Background2_Dataset, Background3_Dataset, Background4_Dataset]:
          ...
          for hName in hNames:
               ...
               effPlot180 = GetCutEfficiencyHisto(Signal_Dataset_180, hName, "kFNormal", **kwargs)
     ...
     for Background_Dataset in [Background1_Dataset, Background2_Dataset, Background3_Dataset, Background4_Dataset]:
          ...
          for hName in ["LdgTop_Mass", "SubLdgTop_Mass"]:
               ...
               # Save canvas under custom dir
               if (Background_Dataset==Background1_Dataset):
                    SaveAs(p, savePath, saveName+"_WindowCutSignificance_Bkg1", kwargs.get("saveFormats"))
               elif (Background_Dataset==Background2_Dataset):
                    SaveAs(p, savePath, saveName+"_WindowCutSignificance_Bkg2", kwargs.get("saveFormats"))
               elif (Background_Dataset==Background3_Dataset):
                    SaveAs(p, savePath, saveName+"_WindowCutSignificance_Bkg3", kwargs.get("saveFormats"))
               elif (Background_Dataset==Background4_Dataset):
                    SaveAs(p, savePath, saveName+"_WindowCutSignificance_Bkg4", kwargs.get("saveFormats"))
                    '''
               # Save canvas under custom dir                                                                                                                                                             
               if (Background_Dataset==Background1_Dataset):
     ...
     '''
     #================================================================================================       
     # Chi Square Probability ( 3 DOF )                                                               
     #================================================================================================           
     print "==============================================================================================="
     print " Chi Square Probability ( 3 DOF )"
     print "==============================================================================================="
     ...
     # Get SavePath                                            
     savePath= GetSavePath(**kwargs)
     saveName = os.path.join(savePath, "ChiSqr_Probability")
     
     chiSqr_Prob, ChiSqr_quantileLow, ChiSqr_quantileHigh = GetChiSquareProbabilityPlot(Background2_Dataset, 3, parseOpts.alpha)

     ProbPlots=[chiSqr_Prob]

     # Create the plot with all the significance plots                                                                           
     p = plots.PlotBase(ProbPlots, kwargs.get("saveFormats"))
     ...
     p.getFrame().GetYaxis().SetLabelSize(18)
     p.getFrame().GetXaxis().SetLabelSize(20)
     p.getFrame().GetXaxis().SetRangeUser(0,20)
     if kwargs.get("createRatio"):
          p.getFrame2().GetYaxis().SetTitle("Ratio")
          p.getFrame2().GetYaxis().SetTitleOffset(1.6)
     ...
     _kwargs= {"createRatio": False, "logX": False, "logY": False, "gridX": True, "gridY": True}
     SetLogAndGrid(p, **_kwargs)
     #p.getFrame().GetYaxis().SetRangeUser(1E-4, 5)

     # Add cut line/box                                                                                                          
     _kwargs = { "lessThan": kwargs.get("cutLessThan")}
     p.addCutBoxAndLine(cutValue=ChiSqr_quantileLow, fillColor=kwargs.get("cutFillColour"), lineColor=ROOT.kBlue-7, box=False, line=kwargs.get("cutLine"), **_kwargs)
     p.addCutBoxAndLine(cutValue=ChiSqr_quantileHigh, fillColor=kwargs.get("cutFillColour"), lineColor=ROOT.kRed, box=False, line=kwargs.get("cutLine"), **_kwargs)

     histograms.addText(0.66, 0.87, "#alpha     =  %s" % (parseOpts.alpha))
     histograms.addText(0.66, 0.79, "#chi^{2}_{#alpha,3}  = "+str("{0:.3f}".format(ChiSqr_quantileLow)), color=ROOT.kBlue-7)
     histograms.addText(0.66, 0.69, "#chi^{2}_{1-#alpha,3}= "+str("{0:.3f}".format(ChiSqr_quantileHigh)), color=ROOT.kRed)
