...
def main(opts):
    ...
    for opt in optModes:
        ...
        datasetsToRemove = ["QCD-b", "Charged"]#, "QCD_HT50to100", "QCD_HT100to200"]#, "QCD_HT200to300"]#, "QCD_HT300to500"]
        for d in datasetsMgr.getAllDatasets():
...
def GetHistoKwargs(h, opts):
    ...
    _moveLegend =  {"dx": -0.08, "dy": -0.01, "dh": -0.1}
    logY    = True
    _yLabel = "Events / %.0f "
    yMin    = 1e0
    if logY:
    ...
    if "WMass" in h:
        ...
        kwargs["opts"]   = {"xmin": 0.0, "xmax": 350.0, "ymin": yMin, "ymaxfactor": yMaxF}
    ...
    if "phi" in h.lower():
        kwargs["ylabel"] = "Events / %.2f "
        kwargs["cutBox"] = {"cutValue": 0.0, "fillColor": 16, "box": False, "line": True, "greaterThan": True}
        kwargs["opts"]   = {"xmin": -3.5, "xmax": +3.5, "ymin": yMin, "ymaxfactor": yMaxF}
        kwargs["xlabel"] = "#phi"
    ...
    if "NBJets" in h:
        ...
        kwargs["cutBox"] = {"cutValue": 3, "fillColor": 16, "box": False, "line": False, "greaterThan": True}

    if "NJets" in h:
        kwargs["xlabel"] = "jet multiplicity"
        kwargs["opts"]   = {"xmin": 0.0, "xmax": 18.0, "ymin": yMin, "ymaxfactor": yMaxF}
        kwargs["cutBox"] = {"cutValue": 7, "fillColor": 16, "box": False, "line": False, "greaterThan": True}
        if "Selections" in h:
            kwargs["moveLegend"] = {"dx": -0.52, "dy": -0.0, "dh": 0.0}

    if "NVertices" in h:
        kwargs["xlabel"] = "PV multiplicity"
        kwargs["cutBox"] = {"cutValue": 1.0, "fillColor": 16, "box": True, "line": False, "greaterThan": True}
        if "AllSelections" in h:
...
if __name__ == "__main__":
    ...
    # Sanity check
    allowedFolders = ["SystTopBDT", "counters", "counters/weighted", "Weighting", "ForDataDrivenCtrlPlots", 
