...
def main(opts, signalMass):
    ...
    for opt in optModes:
        ...
        folderTarg     = "topSelectionBDT_" #topSelectionBDT_
        folderRef      = folderTarg
        ...
        for i in range(len(histoPathsTarg)):
            hT = histoPathsTarg[i]
            hR = hT #histoPathsRef[i]
            if "vs" in hT.lower():
                continue
            #Plot_Comparisons(datasetsMgr, datasetsMgrRef, hT, hR, intLumi)
            Plot_LostEvents(datasetsMgr, datasetsMgrRef, hT, hR, intLumi)
...
def Plot_LostEvents(datasetsMgr, datasetsMgrRef, hT, hR, intLumi):
    ...
    p0T = plots.MCPlot(datasetsMgr, hT, normalizeToLumi=opts.intLumi, saveFormats=[], **_kwargs)
    HT_ = p0T.histoMgr.getHisto(datasetTarg.getName()).getRootHisto().Clone("target")

    p0R = plots.MCPlot(datasetsMgr, hR, normalizeToLumi=opts.intLumi, saveFormats=[], **_kwargs)
    HR_ = p0R.histoMgr.getHisto(datasetRef.getName()).getRootHisto().Clone("reference")
    ...
    p0 = plots.PlotBase([histograms.Histo(myHisto,"h_target-h_ref", "pl", "PL")], saveFormats=[])
    ...
    plots.drawPlot(p0,
                   ...
                   opts         = {"ymin": 0.1, "ymaxfactor": 1.3},
                   opts2        = {"ymin": 0.6, "ymax": 1.4},
                   cutBox       = {"cutValue": 0, "fillColor": 16, "box": False, "line": True, "greaterThan": True}
                   )
