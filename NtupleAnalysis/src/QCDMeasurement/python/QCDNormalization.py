...
class QCDNormalizationTemplate:
    ...
    def plot(self):
        ...
        plot.createFrame(self._plotDirName+"/template_"+self._name.replace(" ","_")+"_"+self._binLabel, opts={"ymin": 0.1, "ymaxfactor": 2.})
        plot.getFrame().GetXaxis().SetTitle("MET (GeV)")
        plot.getFrame().GetYaxis().SetTitle("N_{events}")
        histograms.addStandardTexts(cmsTextPosition="outframe")
        histograms.addText(0.36,0.89, "Integral = %d events"%int(self._histo.Integral(0,-1)*self._normalizationFactor+0.5))
    ...
    def doFit(self, fitOptions="S", createPlot=True):
        ...
        if self._fitFunction == None:
            ...
            if createPlot:
                ...
                plot.createFrame(self._plotDirName+"/fit_"+self._name.replace(" ","_")+"_"+self._binLabel, opts={"ymin": 1e-5, "ymaxfactor": 2.})
                plot.getFrame().GetXaxis().SetTitle("MET (GeV)")
                plot.getFrame().GetYaxis().SetTitle("N_{events}")
...
## for 3-prongs
 ...
 ## original
    ...
    def _makePlot(self, binLabel, histogramDictionary={}):
        ...
        plot.createFrame(self._plotDirName+"/finalFit_"+"_"+binLabel, opts={"ymin": 1e-5, "ymaxfactor": 2.})
        plot.getFrame().GetXaxis().SetTitle("MET (GeV)")
        plot.getFrame().GetYaxis().SetTitle("N_{events}")
