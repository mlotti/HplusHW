'''
DESCRIPTION:
This is the second attempt (v2) to generate datacards for H+ -> tb analysis, in 
...
if OptionGenuineTauBackgroundSource == "DataDriven": #fixme: what is this for?
    # EWK genuine taus from embedding
    embeddingNuisances = myEmbeddingShapeSystematics[:] + myEmbDataDrivenNuisances[:] #fixme: can i remove this?
    DataGroups.append(DataGroup(label             = "CMS_Hptntj_EWK_Tau", 
                                landsProcess      = 3, 
                                shapeHistoName    = shapeHistoName, histoPath=histoPathInclusive,
                                datasetType       = "Embedding", 
                                datasetDefinition = "Data", 
                                validMassPoints   = MassPoints, 
                                nuisances         = embeddingNuisances
                                )
                      )
else:
    # TT
