...
kwargs = {
    ...
    "optMode"          : "",
    "savePath"         : "/publicweb/a/aattikis/",
    "saveFormats"      : [".pdf"],
    "xlabel"           : "", #b-tag SF
    "ylabel"           : "Cumulative selection efficiency",
    "rebinX"           : 1,
    "rebinY"           : 1,
    "xlabelsize"       : 10,
    "ylabelsize"       : None,
    ...
    "removeLegend"     : False,
    "moveLegend"       : {"dx": -0.55, "dy": 0.0, "dh": 0.35},
    "cutValue"         : 1.2,
    "cutLine"          : False,
    "cutBox"           : False,
    "cutLessthan"      : False,
...
def main(hName, opts):
    ...
    p = plots.DataMCPlot(datasetsMgr, hName, normalizeToLumi=1.0)
    
    # Create a comparison plot
    ratioOpts = {"ymin": 0.0, "ymax": 2.0}
    if kwargs.get("logY")==True:
        canvOpts = {"xmin": 0.0, "ymin": 1e-5, "ymaxfactor": 10} #"xmin": 6.0 for main counter
    else:
        canvOpts = {"ymin": 0.0, "ymaxfactor": 1.2}

    # Draw a customised plot & Save it
    plots.drawPlot(p, 
                   "/publicweb/a/aattikis/" + hName.replace("/", "_").replace(" ", "_").replace("(", "_").replace(")", ""),
...
#                   cutBox=None #{"cutValue": kwargs.get("cutValue"), "fillColor": kwargs.get("cutFillColour"), "box": kwargs.get("cutBox"), "line": kwargs.get("cutLine"), "lessThan": kwargs.get("cutLessthan")},
                   )
