    ...
    for opt in optModes:
        ...
        for bType in bTypes:
            if 0:
                ...
                PurityTripletPlots(datasetsMgr, "Inverted", bType)
...
def PurityTripletPlots(datasetsMgr, analysisType="Inverted", bType=""):
    '''
    ...
    histoNames.append("%s/%s_FailedBJetPt_AfterAllSelections" % (folder, analysisType) )
    ...
    histoNames.append("%s/%s_FailedBJetEta_AfterAllSelections" % (folder, analysisType) )
    kwargs = copy.deepcopy(_kwargs)
    kwargs["xlabel"] = "jet #eta"
    kwargs["ylabel"] = "Events / %.2f"
    histoKwargs["%s/%s_FailedBJetEta_AfterAllSelections" % folder] = kwargs

    histoNames.append("%s/%s_FailedBJetBDisc_AfterAllSelections" % (folder, analysisType) )
    kwargs = copy.deepcopy(_kwargs)
    kwargs["xlabel"] = "jet b-tag discriminator"
    kwargs["ylabel"] = "Events / %.2f"
    kwargs["cutBox"] = {"cutValue": 0.8484, "fillColor": 16, "box": True, "line": True, "greaterThan": False}
    histoKwargs["%s/%s_FailedBJetBDisc_AfterAllSelections" % folder] = kwargs
        
    histoNames.append("%s/%s_FailedBJetPdgId_AfterAllSelections" % (folder, analysisType) )
    kwargs = copy.deepcopy(_kwargs)
    kwargs["xlabel"] = "jet pdgId"
    kwargs["ylabel"] = "Events / %.0f"
    histoKwargs["%s/%s_FailedBJetPdgId_AfterAllSelections" % folder] = kwargs
    
    histoNames.append("%s/%s_FailedBJetPartonFlavour_AfterAllSelections" % (folder, analysisType) )
    kwargs = copy.deepcopy(_kwargs)
    kwargs["xlabel"] = "jet parton flavour"
    kwargs["ylabel"] = "Events / %.0f"
    histoKwargs["%s/%s_FailedBJetPartonFlavour_AfterAllSelections" % folder] = kwargs
    
    histoNames.append("%s/%s_FailedBJetHadronFlavour_AfterAllSelections" % (folder, analysisType) )
    kwargs = copy.deepcopy(_kwargs)
    kwargs["xlabel"] = "jet hadron flavour"
    kwargs["ylabel"] = "Events / %.0f"
    histoKwargs["%s/%s_FailedBJetHadronFlavour_AfterAllSelections" % folder] = kwargs

    histoNames.append("%s/%s_FailedBJetAncestry_AfterAllSelections" % (folder, analysisType) ) 
    kwargs = copy.deepcopy(_kwargs)
    kwargs["xlabel"] = "ancestor bit"
    kwargs["ylabel"] = "Events / %.0f"
    histoKwargs["%s/%s_FailedBJetAncestry_AfterAllSelections" % folder] = kwargs
...
def PurityPlots(datasetsMgr, histoName, analysisType="Inverted"):
    ...
    p = plots.ComparisonManyPlot(getPurityHisto(FakeB, Data, inclusiveBins=True), [getPurityHisto(EWKGenuineB, Data, inclusiveBins=True)], saveFormats=[])
    ...
    # Set draw style
    p.histoMgr.setHistoDrawStyle(analysisType + "-FakeB", "LP")
    p.histoMgr.setHistoDrawStyle(analysisType + "-EWK"  , "LP")

    # Set legend style
    p.histoMgr.setHistoLegendStyle(analysisType + "-FakeB", "LP")
    p.histoMgr.setHistoLegendStyle(analysisType + "-EWK"  , "LP")
    ...
    p.histoMgr.setHistoLegendLabelMany({
            ...
            analysisType + "-FakeB" : "FakeB", #=Data-EWK",
            analysisType + "-EWK" : "EWK",
            # analysisType + "-Data": "%s (Data)" % (analysisType),
            # analysisType + "-FakeB" : "%s (FakeB)"  % (analysisType),
    ...
    plots.drawPlot(p, 
                   ...
                   rebinX        = 1,
                   cmsExtraText  = "Preliminary", 
                   createLegend  = {"x1": 0.75, "y1": 0.80, "x2": 0.92, "y2": 0.92},
                   opts          = _opts,
...
def getPurityHisto(histo, refHisto, inclusiveBins=False, printValues=False):
    ...
    # For-loop: Bins    
    for j in range(1, refHisto.GetNbinsX()+1):
