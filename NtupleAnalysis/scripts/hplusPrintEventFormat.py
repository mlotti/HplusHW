...
def findImplementation(files, name):
    for key in files.keys():
        if key.endswith("Generated.cc"):
            ...
            if name.startswith(particle): 
                newname = name.replace(particle+"s_","").replace(particle+"_","")
    ...
    for key in files.keys():
        if key.endswith("Generated.h"):
            ...
            if name.startswith(particle):
                shortname = name.replace("%ss_"%particle,"").replace("%s_"%particle,"")
                suffix = None
                ...
                if suffix != None:
                    ...
                    for line in files[key]:
                        n = line.split(":")
                        if n[1].startswith("  Particle<ParticleCollection<double>>"):
                            if suffix in line:
