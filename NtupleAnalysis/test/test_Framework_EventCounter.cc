...
TEST_CASE("EventCounter works", "[Framework]") {
  ...
  SECTION("Misuse with main counter") {
    TDirectory dir("rootdir", "rootdir");
    ec.setOutput(&dir);
    REQUIRE_THROWS_AS( ec.addCounter("count1"), std::logic_error );
  }

  SECTION("Misuse with subcounter") {
    TDirectory dir("rootdir", "rootdir");
    ec.setOutput(&dir);
