...
class PseudoMultiCrabCreator:
    ...
    def addModule(self, module, altMode=False):
        ...
        if self._energy == None:
            ...
            self._writeRootFileToDisk(self._currentSubTitle, altMode)
    ...
    def _writeRootFileToDisk(self, subTitle, altMode=False):
        ...
        if not altMode:
            self._rootFilePath = "%s/%s/res/histograms-%s.root" % (self._myBaseDir,self._title+self._mySubTitles[-1],self._title+self._mySubTitles[-1])
