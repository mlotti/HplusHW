...
status_re = re.compile("(?P<id>\d+)\s+(?P<end>\S)\s+(?P<status>\S+)\s+(?P<action>\S+)\s+(?P<execode>\S+)?\s+(?P<jobcode>\S+)?\s+(?P<host>\S+)?")
...
class CrabJob:
    ...
    def failed(self, what):
        if (what == "all" or what == "aborted") and self.origStatus == "Aborted":
            return True
...
if __name__ == "__main__":
    ...
    if opts.resubmit not in ["all", "aborted"]:
