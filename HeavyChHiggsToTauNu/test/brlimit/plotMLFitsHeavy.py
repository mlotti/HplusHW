...
def doBkgFitPlots(mlfit,lumi):
    ...
    def createDrawPlot(gr, labels, fname):
        ...
        for i, label in enumerate(labels):
                y = gr.GetY()[i]-0.3
                new_label = string.replace(label,"BinByBin", "Bin")            
                l.DrawLatex(x_nuis, y, new_label[:20])
                l.DrawLatex(x_value, y, "%.2f #pm %.2f" % (gr.GetX()[i], gr.GetErrorX(i)))
        
        plot.save()
    
    (gr, labels) = mlfit.fittedGraphHeavy(firstMass, backgroundOnly=True)
    createDrawPlot(gr, labels, "mlfit_backgroundOnly_heavy")

    try:
        (gr, labels, shapeStatNuisance) = mlfit.fittedGraphShapeStat(firstMass, backgroundOnly=True)
        createDrawPlot(gr, labels, "mlfit_backgroundOnlyShapeStat_heavy")
        (gr, labels) = mlfit.fittedGraphShapeBinByBinHeavy(firstMass, backgroundOnly=True)
        createDrawPlot(gr, labels, "mlfit_backgroundOnlyBinByBin_heavy")
    except Exception, e:
