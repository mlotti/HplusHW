...
h = plots.DataMCPlot(datasets, "signalAnalysisTauSelectionHPSTightTauBased/TauID_RtauCut")
...
h.createFrame("TauIdRtau_afterTauId", ymin=0.01, ymaxfactor=2)
...
h.stackMCHistograms()
h.addMCUncertainty()
h.createFrameFraction("TauIdRtau_afterTauId2")
h.createFrameFraction("TauIdRtau_afterTauId2", opts={"ymin": 0.01, "ymaxfactor": 2})
ROOT.gPad.SetLogy(True)
h.frame.GetXaxis().SetTitle("p_{T}^{leading track} (GeV/c)")
h.frame.GetYaxis().SetTitle("Events / 5 GeV/c")
h.setLegend(histograms.createLegend())
