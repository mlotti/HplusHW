...
h = plots.DataMCPlot(datasets, "signalAnalysisTauSelectionHPSTightTauBased/TauEmbeddingAnalysis_afterTauId_leadPFChargedHadrPt")
...
h.createFrame("leadPFChargedHadrPtEmbedding_afterTauId", ymin=0.01, ymaxfactor=2)
h.frame.GetXaxis().SetTitle("p_{T}^{leading track} (GeV/c)")
...
# the data/MC
h = plots.DataMCPlot(datasets, "signalAnalysisTauSelectionHPSTightTauBased/TauEmbeddingAnalysis_begin_leadPFChargedHadrPt")
h.stackMCHistograms()
h.addMCUncertainty()
h.createFrameFraction("leadPFChargedHadrPtEmbedding_afterTauId2",)
h.frame.GetXaxis().SetTitle("p_{T}^{leading track} (GeV/c)")
h.frame.GetYaxis().SetTitle("Events / 5 GeV/c")
h.setLegend(histograms.createLegend())
h.draw()
