...
h = plots.DataMCPlot(datasets, "signalAnalysisTauSelectionHPSTightTauBased/TauEmbeddingAnalysis_afterTauId_TransverseMassOriginal")
...
#h.createFrame("MtEmbedding_afterTauId", ymin=0.001, ymaxfactor=2)
h.createFrame("MtOriginal_afterTauId")
h.frame.GetXaxis().SetTitle("m_{T}(#tau jet, MET) (GeV/c^{2})")
h.frame.GetYaxis().SetTitle("Events / 5 GeV/c^{2}")
...
h.stackMCHistograms()
h.addMCUncertainty()
h.createFrameFraction("MtOriginal_afterTauId2")
#h.cf.pad1.SetLogy(True)
#ROOT.gPad.SetLogy(True)
h.frame.GetXaxis().SetTitle("m_{T}(#tau jet, MET) (GeV/c^{2})")
h.frame.GetYaxis().SetTitle("Events / 5 GeV/c^{2}")
h.setLegend(histograms.createLegend())
