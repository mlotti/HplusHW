...
h = plots.DataMCPlot(datasets, "signalAnalysisTauSelectionHPSTightTauBased/TauEmbeddingAnalysis_begin_originalMuonPt")
...
h.createFrame("muonPt_begin", ymin=0.01, ymaxfactor=2)
...
h.stackMCHistograms()
h.addMCUncertainty()
h.createFrameFraction("muonPt_begin2",)
h.frame.GetXaxis().SetTitle("p_{T}^{#mu} (GeV/c)")
h.frame.GetYaxis().SetTitle("Events / 5 GeV/c")
h.setLegend(histograms.createLegend())
h.draw()
