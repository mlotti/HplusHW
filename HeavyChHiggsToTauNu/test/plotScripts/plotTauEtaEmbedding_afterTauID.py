...
h = plots.DataMCPlot(datasets, "signalAnalysisTauSelectionHPSTightTauBased/TauEmbeddingAnalysis_afterTauId_selectedTauEta")
...
h.createFrame("tauEtaEmbedding_afterTauId", ymin=0.01, ymaxfactor=2)
h.frame.GetXaxis().SetTitle("#eta^{#tau jet}")
...
h.stackMCHistograms()
h.addMCUncertainty()
h.createFrameFraction("tauEtaEmbedding_afterTauId2", opts={"ymin": 0.1, "ymaxfactor": 4})
ROOT.gPad.SetLogy(True)
h.frame.GetXaxis().SetTitle("#eta^{#tau jet}")
h.frame.GetYaxis().SetTitle("Events")
h.setLegend(histograms.createLegend())
