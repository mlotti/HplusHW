...
h = plots.DataMCPlot(datasets, "signalAnalysisTauSelectionHPSTightTauBased/TauEmbeddingAnalysis_afterMetCut_originalMuonPt")
...
h.createFrame("muonPt_afterMetCut", ymin=0.01, ymaxfactor=2)
h.frame.GetXaxis().SetTitle("p_{T}^{#mu} (GeV/c)")
...
h.stackMCHistograms()
h.addMCUncertainty()
h.createFrameFraction("muonPt_afterMetCut2",)
h.frame.GetXaxis().SetTitle("p_{T}^{#mu} (GeV/c)")
h.frame.GetYaxis().SetTitle("Events / 5 GeV/c")
h.setLegend(histograms.createLegend())
h.draw()
