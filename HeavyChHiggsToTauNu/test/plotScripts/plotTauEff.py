...
def plot(datasets):
    ptSelected = plots.PlotBase([datasets.getDataset("TTToHplusBWB_M120").getDatasetRootHisto(analysis+"/TauSelection/TauSelection_selected_taus_pt")])
    ptAll = plots.PlotBase([datasets.getDataset("TTToHplusBWB_M120").getDatasetRootHisto(analysis+"/TauSelection/TauSelection_all_tau_candidates_pt")])
    ptClean = plots.PlotBase([datasets.getDataset("TTToHplusBWB_M120").getDatasetRootHisto(analysis+"/TauSelection/TauSelection_cleaned_tau_candidates_pt")])
    ...
    ptSelected.histoMgr.forEachHisto(lambda h: h.getRootHisto().Rebin(2))    
    hptSelected = ptSelected.histoMgr.getHisto("TTToHplusBWB_M120").getRootHisto().Clone(analysis+"/TauSelection/TauSelection TauSelection_selected_taus_pt")
    
    ptAll._setLegendStyles()
    ptAll._setLegendLabels()
    ptAll.histoMgr.setHistoDrawStyleAll("P")
    ptAll.histoMgr.forEachHisto(lambda h: h.getRootHisto().Rebin(2))    
    hptAll = ptAll.histoMgr.getHisto("TTToHplusBWB_M120").getRootHisto().Clone(analysis+"/TauSelection/TauSelection_all_tau_candidates_pt")

    ptClean._setLegendStyles()
    ptClean._setLegendLabels()
    ptClean.histoMgr.setHistoDrawStyleAll("P")
    ptClean.histoMgr.forEachHisto(lambda h: h.getRootHisto().Rebin(2))    
    hptClean = ptClean.histoMgr.getHisto("TTToHplusBWB_M120").getRootHisto().Clone(analysis+"/TauSelection/TauSelection_cleaned_tau_candidates_pt")
...
#    tex5 = ROOT.TLatex(0.55,0.9,"tt+jets") 
    ...
    canvas3.Print("TauEff_pt_h120.png")
    canvas3.Print("TauEff_pt_h120.C")


### tt 
    ptSelected2 = plots.PlotBase([datasets.getDataset("TTJets").getDatasetRootHisto(analysis+"/TauSelection/TauSelection_selected_taus_pt")])
    ptAll2 = plots.PlotBase([datasets.getDataset("TTJets").getDatasetRootHisto(analysis+"/TauSelection/TauSelection_all_tau_candidates_pt")])
    ptClean2 = plots.PlotBase([datasets.getDataset("TTJets").getDatasetRootHisto(analysis+"/TauSelection/TauSelection_cleaned_tau_candidates_pt")])
    ...
    ptSelected2.histoMgr.forEachHisto(lambda h: h.getRootHisto().Rebin(2))    
    hptSelected2 = ptSelected2.histoMgr.getHisto("TTJets").getRootHisto().Clone(analysis+"/TauSelection/TauSelection TauSelection_selected_taus_pt")
    
    ptAll2._setLegendStyles()
    ptAll2._setLegendLabels()
    ptAll2.histoMgr.setHistoDrawStyleAll("P")
    ptAll2.histoMgr.forEachHisto(lambda h: h.getRootHisto().Rebin(2))    
    hptAll2 = ptAll2.histoMgr.getHisto("TTJets").getRootHisto().Clone(analysis+"/TauSelection/TauSelection_all_tau_candidates_pt")

    ptClean2._setLegendStyles()
    ptClean2._setLegendLabels()
    ptClean2.histoMgr.setHistoDrawStyleAll("P")
    ptClean2.histoMgr.forEachHisto(lambda h: h.getRootHisto().Rebin(2))    
    hptClean2 = ptClean2.histoMgr.getHisto("TTJets").getRootHisto().Clone(analysis+"/TauSelection/TauSelection_cleaned_tau_candidates_pt")
    
    
    hptEff2 = hptSelected2.Clone("hptSelected2")
    hptEff2.SetName("hptEff2")
    hptEff2.SetTitle("TauVetoEff2")
    hptEff2.Divide(hptAll2)
    
    hptIsolEff2 = hptSelected2.Clone("hptSelected2")
...
#    hptIsolEff.SetMaximum(0.1)
    ...
    tex1.SetTextSize(20)
    tex1.Draw()    
    marker1 = ROOT.TMarker(0.3,0.31,hptEff2.GetMarkerStyle())
    marker1.SetNDC()
    marker1.SetMarkerColor(hptEff2.GetMarkerColor())
    marker1.SetMarkerSize(0.9*hptEff2.GetMarkerSize())
    marker1.Draw()
    ...
    tex2.SetTextSize(20)
    tex2.Draw()    
    marker2 = ROOT.TMarker(0.3,0.251,hptIsolEff2.GetMarkerStyle())
    marker2.SetNDC()
    marker2.SetMarkerColor(hptIsolEff2.GetMarkerColor())
    marker2.SetMarkerSize(0.9*hptIsolEff2.GetMarkerSize())
    marker2.Draw()
...
#    tex5 = ROOT.TLatex(0.55,0.9,"Signal, m_{H^{#pm}} = 120 GeV/c^{2}")
    ...
    canvas5.Print("TauEff_pt_tt.png")
    canvas5.Print("TauEff_pt_tt.C")


##########################
    etaSelected = plots.PlotBase([datasets.getDataset("TTToHplusBWB_M120").getDatasetRootHisto(analysis+"/TauSelection/TauSelection_selected_taus_eta")])
    etaAll = plots.PlotBase([datasets.getDataset("TTToHplusBWB_M120").getDatasetRootHisto(analysis+"/TauSelection/TauSelection_all_tau_candidates_eta")])
    etaClean = plots.PlotBase([datasets.getDataset("TTToHplusBWB_M120").getDatasetRootHisto(analysis+"/TauSelection/TauSelection_cleaned_tau_candidates_eta")])
    ...
    etaSelected.histoMgr.forEachHisto(lambda h: h.getRootHisto().Rebin(4))    
    hetaSelected = etaSelected.histoMgr.getHisto("TTToHplusBWB_M120").getRootHisto().Clone(analysis+"/TauSelection/TauSelection_selected_taus_eta")
    
    etaAll._setLegendStyles()
    etaAll._setLegendLabels()
    etaAll.histoMgr.setHistoDrawStyleAll("P")
    etaAll.histoMgr.forEachHisto(lambda h: h.getRootHisto().Rebin(4))    
    hetaAll = etaAll.histoMgr.getHisto("TTToHplusBWB_M120").getRootHisto().Clone(analysis+"/TauSelection/TauSelection_all_tau_candidates_eta")

    etaClean._setLegendStyles()
    etaClean._setLegendLabels()
    etaClean.histoMgr.setHistoDrawStyleAll("P")
    etaClean.histoMgr.forEachHisto(lambda h: h.getRootHisto().Rebin(4))    
    hetaClean = etaClean.histoMgr.getHisto("TTToHplusBWB_M120").getRootHisto().Clone(analysis+"/TauSelection/TauSelection_cleaned_tau_candidates_eta")
...
#    canvas3.SetLogy()
    ...
    tex5 = ROOT.TLatex(0.25,0.9,"Signal, m_{H^{#pm}} = 120 GeV/c^{2}")
#    tex5 = ROOT.TLatex(0.55,0.9,"tt+jets")
    tex5.SetNDC()
    tex5.SetTextSize(18)
    tex5.Draw()
    
    tex1 = ROOT.TLatex(0.25,0.8,"Total selection")
    tex1.SetNDC()
    tex1.SetTextSize(20)
    tex1.Draw()
    
    marker1 = ROOT.TMarker(0.2,0.815,hetaEff.GetMarkerStyle())
    marker1.SetNDC()
    marker1.SetMarkerColor(hetaEff.GetMarkerColor())
    marker1.SetMarkerSize(0.9*hetaEff.GetMarkerSize())
    marker1.Draw()   
    tex2 = ROOT.TLatex(0.25,0.7,"Isolation") 
    tex2.SetNDC()
    tex2.SetTextSize(20)
    tex2.Draw()    
    marker2 = ROOT.TMarker(0.2,0.715,hetaIsolEff.GetMarkerStyle())
    marker2.SetNDC()
    ...
    canvas2.Print("TauEff_eta_h120.png")
    canvas2.Print("TauEff_eta.C")  
