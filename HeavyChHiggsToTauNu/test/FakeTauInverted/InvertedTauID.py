...
#        plot.histoMgr.setHistoLegendLabelMany({"Inv": h1.GetTitle(), "Base": h2.GetTitle()})
 ...
 #           plot.getPad1().SetLogy(False)
        ...
        histograms.addStandardTexts(cmsTextPosition="right")
...
#                return QCDEWKFunction(x,par,1)
        ...
        """
        histograms.addText(0.4,0.77,"Data, Inverted TauID")
...
#	        return TestFunction(x,par,1)
	...
	thePlot.Draw("same")
        ...
        histograms.addText(0.2,0.2,"EWK MC, "+name.replace("base","Base")+" TauID")
	...
	self.normEWK_FakeTaus = histo.Integral(0,histo.GetNbinsX())
        ...
        if name == "Inverted":
            self.nMCEWKinverted_FakeTaus = self.normEWK_FakeTaus
        if name == "baseline":
            self.nMCEWKbaseline_FakeTaus = self.normEWK_FakeTaus
	...
	thePlot.Draw("same")
        ...
        print "EWK MC Fake Taus",fitPars
 ...
 #       histo.GetXaxis().SetTitle("MET  (GeV)")
        ...
        histograms.addText(0.35,0.77,"Data, Baseline TauID")
...
#        nEWKinverted = nDataInverted*(1-QCDfractionInInvertedEvents)
            ...
            self.normalizationForInvertedEWKEvents_FakeTaus = nMCEWKbaseline_FakeTaus/nMCEWKinverted_FakeTaus
...
# 	print "Normalization for inverted QCD events error   ",normalizationForInvertedEventsError                                                  
	...
	fOUT.write("QCDInvertedNormalization = {\n")
	    ...
	    if i < len(self.normFactors) - 1 or len(self.normFactorsEWK) > 0 or len(self.normFactorsEWK_FakeTaus) > 0:
		line += ","
	    line += "\n"
            fOUT.write(line)
            i = i + 1
	    ...
	    if i < len(self.normFactorsEWK) - 1 or len(self.normFactorsEWK_FakeTaus) > 0:
		line += ","
	    line += "\n"
            fOUT.write(line)
            i = i + 1
