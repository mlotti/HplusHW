...
void plot(int mass) {
  ...
  transverseMass->SetBottomMargin(0.08);
...
/*  hFrame->GetXaxis()->SetLabelSize(0.05);
  ...
  TH1* hExpBkgTotalUncert = (TH1*)hExpBkg->Clone();
  ...
  for (int i = 1; i <= hFrame->GetNbinsX(); ++i) {
    double myQCDTotalUncert = TMath::Power(qcd->GetBinError(i), 2)
      + TMath::Power(qcd->GetBinContent(i)*myQCDRelUncert[i-1], 2);
    double myEWKTotalUncert = TMath::Power(ewktau->GetBinError(i), 2)
      + TMath::Power(ewktau->GetBinContent(i)*myEWKRelUncert[i-1], 2);
    double myFakesTotalUncert = TMath::Power(fakes->GetBinError(i), 2)
      + TMath::Power(fakes->GetBinContent(i)*myFakesRelUncert[i-1], 2);
    ...
    cout << "  bkg. stat. uncert. " << hExpBkg->GetBinError(i) << ", stat+syst uncert. " << TMath::Sqrt(myQCDTotalUncert + myEWKTotalUncert + myFakesTotalUncert) << endl;
  }
