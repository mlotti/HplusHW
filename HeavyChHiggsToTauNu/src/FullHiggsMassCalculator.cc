...
namespace HPlus {
  ...
  const TauSelection::Data tauData, const BTagging::Data bData, const METSelection::Data metData) {
    ...
    // 2) Define b-jet, tau, and MET momentum vectors
    TVector3 myBJetVector(myHiggsSideBJet->px(), myHiggsSideBJet->py(), myHiggsSideBJet->pz());
    TVector3 myVisibleTauVector(tauData.getSelectedTau()->px(), tauData.getSelectedTau()->py(), tauData.getSelectedTau()->pz());
    ...
    doCalculate(myVisibleTauVector, myBJetVector, myMETVector, output);

    // 4) Classify event according to what was identified correctly and what was not
    //    Histograms are filled accordingly (flag "doHistogramming" appears here!)
    //If MC, match to GEN particles to see if it was the correct one
    //bool myMatchStatus = false;
    //if (!iEvent.isRealData())
    //  myMatchStatus = doMCMatching(iEvent, tauData.getSelectedTau(), myHiggsSideBJet, output);
  ...
  edm::Ptr<pat::Jet> FullHiggsMassCalculator::findHiggsSideBJet(const BTagging::Data bData, const TauSelection::Data tauData) {
    ...
    for (edm::PtrVector<pat::Jet>::iterator iBjet = bData.getSelectedJets().begin();
	 iBjet != bData.getSelectedJets().end(); ++iBjet) {
      // Calculate the distance in (eta, phi) between them and the selected tau. Choose the b-jet that is closest to the tau
  ...
  void FullHiggsMassCalculator::calculateNeutrinoPz(TVector3& pTau, TVector3& pB, TVector3& MET, 
    ...
    */
    ...
    double neutrinoPzSolution1;
    double neutrinoPzSolution2;
    ...
    output.fNeutrinoZSolution = neutrinoPzSolution1; // TODO: which solution should be selected???
  }

//   void calculateTopMass() {
    
//   }

//   void calculateHiggsMass() {

//   }

//   void doEventClassification() {
...
//   }
  ...
  void FullHiggsMassCalculator::doCalculate(TVector3& tau, TVector3& bjet, TVector3& met, FullHiggsMassCalculator::Data& output) {
