...
namespace HPlus {
  QCDMeasurement::QCDMeasurement(const edm::ParameterSet& iConfig, EventCounter& eventCounter, EventWeight& eventWeight):
    ...
    // ftransverseMassCutCount(eventCounter.addCounter("transverseMass cut")),
   {
    ...
    makeTH<TNamed>("parameterSet", iConfig.dump().c_str());
    ...
    hMETFactorizationNJets = makeTH<TH2F>("QCD_METFactorization_NJets", "METFactorizationNJets;tau p_{T}, GeV/c;MET, GeV", 60, 0, 300., 60, 0., 300.);
    ...
    hMETFactorizationBJets = makeTH<TH2F>("QCD_METFactorization_BJets", "METFactorizationBJets;tau p_{T}, GeV/c;MET, GeV", 60, 0, 300., 60, 0., 300.);
    ...
    hStdWeightedRtauWithoutNjetsBeforeCut = makeTH<TH1F>(*fs, "QCD_StdCutPath_Weighted_noNjets_Rtau_before", "Rtau;Rtau;N_{events}/0.05", 24, 0., 1.2);
    hStdWeightedRtauWithoutNjetsAfterCut = makeTH<TH1F>(*fs, "QCD_StdCutPath_Weighted_noNjets_Rtau_after", "Rtau;Rtau;N_{events}/0.05", 24, 0., 1.2);
    ...
    for (int i = 0; i < 4; i++) {
      ...
      hABCDTauIsolBNonWeightedTauPtVsMET[i] = makeTH<TH2F>(myLabel.str().c_str(), "NonWeightedTauPtvsMET;tau p_{T}, GeV/c;MET, GeV", 60, 0, 300., 60, 0., 300.);
      myLabel.str("");
...
//     increment(fTriggerAndHLTMetCutCounter);
    ...
    if (tauDataForTauID.passedEvent()) {
      hStdWeightedRtauWithoutNjetsBeforeCut->Fill(myFactorizationTableIndex, fEventWeight.getWeight());
      if (tauDataForTauID.selectedTauPassedRtau()) {
        hStdWeightedRtauWithoutNjetsAfterCut->Fill(myFactorizationTableIndex, fEventWeight.getWeight());
      }
    }
